...
class KlarnaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($verifyToken['status'] === 'success') {
            ...
            if ($createSource['status'] == true) {
                ...
                $storeSource->transaction_mode = "test";
                $storeSource->save();
    ...
    public function testindex()
    {
        $payment_request = array(
            "token" => "a67a33dd953bca10c0fc8849b63866ee",
            "testMode" => "test",
            "hostName" => "wordtest.oganro.com",
            "gatewayToken" => "klarna_xm97H8wUW",
            "orderId" => '5766',
            "amount" => "19381",
            "currency" => "EUR",
            "first_name" => "Nishanrth",
            "last_name" => "VEema",
            "email" => "nishanth@development.com",
            "city" => "Hawthorn East",
            "country" => "AU",
    ...
    public function verifyToken($data)
    {
        ...
        $getProvider = IPGConfig::where('provider_id', function ($query) use ($data) {
            ...
            ->where('host_name', 'like', $data['hostName'] . '%')->with('client', 'provider')
        ...
        if (count($getProvider) === 0) {
            ...
            //         return array('status' => 'failed', 'data' => array('error' => array('message' => 'You have exceeded the allowed Test transaction!!. Please contact our admin team to enable the Test mode again!!. We are sorry for your inconvenience ')));
            //     }
            // }
